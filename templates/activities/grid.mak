    ...
    <div class="content">
        ...
        <ul class="nav nav-tabs table_tabs">
            ...
            % else:
                <li>
            % endif
                <a class="active" href="${request.route_url('activities_read_many', output='html')}${handle_query_string(request.url, return_value='query_string', remove=['order_by', 'dir', 'status'])}">${_('Deals')}</a>
            </li>
            % if is_moderator:
                % if 'status=pending' in request.path_qs:
                    <li class="active moderator-show-pending-right">
                        <a class="active" href="${handle_query_string(request.current_route_url(), remove=['status'])}" data-toggle="tooltip" title="${_('Show all')}">
                            <i class="icon-flag"></i>
                        </a>
                    </li>
                % else:
                    <li class="moderator-show-pending-right">
                        <a class="active" href="${handle_query_string(request.current_route_url(), add=[('status', 'pending')])}" data-toggle="tooltip" title="${_('Show only pending')}">
                            <i class="icon-flag"></i>
                            </a>
                    </li>
                % endif
            ...
            % else:
                <li>
            % endif
                <a class="active" href="${request.route_url('stakeholders_byactivities_all', output='html')}${handle_query_string(request.url, return_value='query_string', remove=['order_by', 'dir', 'status'])}">${_('Investors')}</a>
            </li>
